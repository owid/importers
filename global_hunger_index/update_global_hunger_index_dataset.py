...
def split_country_groups(data, grouped_countries_name):
    ...
    grouped_countries = grouped_countries_name.replace('*', '').replace('and ', '').split(', ')
    clean = data[~grouped_countries_filter].reset_index(drop=True)
    for country in grouped_countries:
        added = data[data['Country'] == grouped_countries_name].replace({grouped_countries_name: country})
...
def main():
    ...
    data = pd.read_excel(DATA_URL, sheet_name='2021 GHI Ranking', skiprows=2).drop(columns=['Unnamed: 0', 'Rank1']). \
        dropna(how='all')
    ...
    range_values = [value for value in clean_melt[GHI_NAME].unique() if not is_float(value) if '*' in value]
    value_remapping = {value: np.array(value.replace('*', '').split('–')).astype(float).round().mean()
