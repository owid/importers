...
OLD_DATA_URL = (
    "https://github.com/owid/owid-datasets/raw/master/datasets/Global%20Hunger%20Index-%20IFPRI%20(2018)/"
...
OUTPUT_FILE = os.path.join(
    CURRENT_DIR, f"global_hunger_index_sanity_checks_{DATE_TAG}.html"
)
...
def plot_country(comparison, country):
    ...
    fig = px.line(
        plot_data[plot_data["Country"] == country],
        x="Year",
        y="value",
        color="source",
        color_discrete_map={"old": "red", "new": "blue"},
        markers=True,
    )
    fig.update_layout({"title": country}).update_xaxes(
        showgrid=True, title="Year", autorange=False, range=x_range
    ).update_yaxes(
        showgrid=True, title="Global Hunger Index", autorange=False, range=y_range
    )
    return fig
