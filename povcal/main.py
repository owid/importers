...
def medians_by_country_year(df):
    ...
    for country_year_tuple in dfg.groups.keys():
        ...
        median_poverty_line = country_year_df.iloc[[country_year_df.HeadCount.searchsorted(0.5)]]
        median_poverty_line_by_country_year[country_year_tuple] = median_poverty_line.iloc[0].poverty_line

    df = pd.DataFrame.from_dict(median_poverty_line_by_country_year, orient="index").reset_index()
...
def download_data_and_write_csv(poverty_line):
    ...
    payload = {'Countries': 'all', 'YearSelected': 'all',
               'PovertyLine': rounded_poverty_line, 'display': 'C'}
    result = requests.get(
        'http://iresearch.worldbank.org/PovcalNet/PovcalNetAPI.ashx', params=payload, timeout=10)
...
def download_raw_data():
    ...
    for dollar in range(1, 3, 1):
        poverty_lines.extend([round(cent, 2) for cent in arange(dollar - 1, 0.50, 0.01)])

    print(poverty_lines)

    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        while len(poverty_lines):
            future_to_poverty_line = {executor.submit(
                download_data_and_write_csv, poverty_line): poverty_line for poverty_line in poverty_lines}
...
def combine_raw_data():
    data_files = [
        f'data_by_poverty_line/{poverty_line}' for poverty_line in absolute_poverty_lines]
    data_csvs = [pd.read_csv(filename, index_col=None, header=0)
...
def drop_unnecessary_columns(raw_data):
    return raw_data.drop(columns=['isInterpolated', 'useMicroData', 'RegionCode', 'PPP', 'Watts', 'SvyInfoID', 'Polarization', 'PovGapSqr', 'pr.mld'])
...
def add_derived_columns(df):
    df['poverty_absolute'] = (
        df.poverty_percentage * df.ReqYearPopulation * 1000000).astype(int)
