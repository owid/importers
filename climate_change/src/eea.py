...
def ghg_concentrations():
    ...
    eea_data_file = "https://www.eea.europa.eu/data-and-maps/daviz/atmospheric-concentration-of-carbon-dioxide-5/" \
                    "download.csv"
    ...
    eea_data = eea_data.rename(columns={
    ...
    co2_concentration = eea_data.rename(columns={'CO2 (ppm)': co2_column})[['location', 'year', co2_column]].\
        dropna().reset_index(drop=True)
    ch4_concentration = eea_data.rename(columns={'CH4 (ppb)': ch4_column})[['location', 'year', ch4_column]].\
        dropna().reset_index(drop=True)
    n2o_concentration = eea_data.rename(columns={'N2O (ppb)': n2o_column})[['location', 'year', n2o_column]].\
        dropna().reset_index(drop=True)
