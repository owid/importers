...
class ChartRevisionSuggester:
    ...
    def insert(
        self, suggested_chart_revisions: List[dict], suggested_reason: str = None
    ) -> None:
        ...
        try:
            ...
            n_before = db.fetch_one("SELECT COUNT(id) FROM suggested_chart_revisions")[
                0
            ]
    ...
    def _get_charts_from_old_variables(
        ...
        with get_connection().cursor() as cursor:
            ...
            # retrieves charts
            chart_ids_str = ",".join(
                [str(_id) for _id in df_chart_dimensions["chartId"].unique().tolist()]
            )
            columns = [
                "id",
                "config",
    ...
    def _modify_chart_dimensions(
        self, chart_id: int, chart_dimensions: List[dict], chart_config: dict
    ) -> None:
